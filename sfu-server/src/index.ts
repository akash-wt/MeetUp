...
io.on("connection", async (socket) => {
    ...
    socket.on("createWebRtcTransport", async (payload: { roomId: string }, callback: (response: Partial<{
        id: string;
        iceParameters: any;
        iceCandidates: any[];
        dtlsParameters: any
        ...
        try {
            const router = getRoom(payload.roomId); if (!router) return callback({ error: "Room not found" });
    ...
    socket.on(
        "connectTransport",
        async (payload: { roomId: string, transportId: string, dtlsParameters: any }, callback: (response?: { error: string }) => void) => {
            const peer = roomPeers.get(payload.roomId)?.get(socket.id);
            if (!peer) return callback({ error: "Peer not found" });

            const transport = peer.transports.find((t) => t.id === payload.transportId);
    ...
    socket.on(
        "produce",
        async (payload: { roomId: string, transportId: string, kind: mediasoupTypes.MediaKind, rtpParameters: any }, callback: (response: { id: string } | { error: string }) => void) => {
            const peer = roomPeers.get(payload.roomId)?.get(socket.id);
            if (!peer) return callback({ error: "Peer not found" });
    ...
    socket.on(
        "consume",
        async (payload: { roomId: string, transportId: string, producerId: string, rtpCapabilities: any }, callback: (response: { id: string; producerId: string; kind: mediasoupTypes.MediaKind; rtpParameters: any } | { error: string }) => void) => {
